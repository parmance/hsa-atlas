 ...
 */
   ...
   fprintf(fpout, "\nvoid HSA_FUNCTION Mjoin(ATL_%caxpy,PHSA)(const int N, const SCALAR alpha, const TYPE *X, const int incX,\n", pre);
 ...
 */
{
   ...
   for (i=0; i < n; i++)
   {
      ...
      if (pre == 's' || pre == 'd')
      {
         ...
         fprintf(fpout, "#define ATL_UAXPY HSA_FUNCTION Mjoin(%s,PHSA)\n",
                 nam);
         fprintf(fpout, "#else\n");
      }
