 ...
 */
            ...
            if (arch == DHSA)
               flg = NewAppendedString(flg, "-fdirecthsa");
...
/*
 * DHSA target requires -fdirecthsa.
 */
   if (arch == DHSA)
      for (i=0; i < NCOMP; i++)
         usrcomps[i+NCOMP] = NewAppendedString(usrcomps[i+NCOMP],
 ...
 */
{
   ...
   char* flags = (arch == DHSA) ? "-O -fdirecthsa" : "-O";
   if (!gccs)
      return(0);
   for (i=0; gccs[i]; i++)
   {
      ...
      {
         if (!CompTest(verb, targ, GCC_, gccs[i], flags))
         {
            if (CompIsMinGW(gccs[i]) || CompIsAppleGcc(gccs[i]))
               ibest = i+1;
            else if (!CompTest(verb, targ, GCC_, gccs[i], flags))
               return(i+1);
         }
      }
 ...
 */
   if (OSpaths)
   {
      ...
      i = SelectBestGcc(verb, targ, GMAJOR, GMINOR, gccs, USEMINGW, arch);
      if (i > 1)
      {
         free(ln);
 ...
 */
   ...
   if (sp)
   {
      ...
      gccs = GetLinesFromFile(atlsys(NULL, ln, verb, 1), gccs);
      i = SelectBestGcc(verb, targ, cmaj, cmin, gccs, USEMINGW, arch);
      if (i > 1)
      {
         free(ln);
         return(FreeListGetString(gccs, i-1));
