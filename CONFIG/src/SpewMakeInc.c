...
"#  ---------------------------------------------------------------------\n");
   ...
   if (mach == DHSA)
      fprintf(fpout, "   ATLAShsalib = $(LIBdir)/libatlashsa.a\n");
...
*/
   ...
   char* dhsa_link_opt = (mach == DHSA)
      ? "-fdirecthsa -Wl,--whole-archive $(ATLAShsalib) -Wl,--no-whole-archive"
      : "";
   if (THREADS)
   {
      if (USEMINGW || (ptrbits == 32 && OSIsWin(OS)))
         fprintf(fpout, "   LIBS = -lkernel32 -lm %s\n\n", dhsa_link_opt);
      else if (OSIsWin(OS))
         fprintf(fpout, "   LIBS = -lm %s\n\n", dhsa_link_opt);
      else
         fprintf(fpout, "   LIBS = -lpthread -lm %s\n\n", dhsa_link_opt);
   }
   else
      fprintf(fpout, "   LIBS = -lm %s\n\n", dhsa_link_opt);
